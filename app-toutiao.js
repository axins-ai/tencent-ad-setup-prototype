...
const MOCK = {
  ...
  productLibrary: [{
    ...
    name: '5G智享套餐',
    image: '📱',
    sellingPoints: ['高速流量', '全国通话', '免费副卡']
  }, {
    id: 'p_002',
    name: '家庭千兆宽带',
    image: '🌐',
    sellingPoints: ['千兆速率', '稳定不掉线', '免费安装']
  }, {
    id: 'p_003',
    name: '合约机0元购',
    image: '📦',
    sellingPoints: ['0元购机', '月租返还', '免息分期']
  }, {
    id: 'p_004',
    name: '移动云盘会员',
    image: '☁️',
    sellingPoints: ['空间扩容', '自动备份', '多端同步']
  }, {
    id: 'p_005',
    name: '权益会员包',
    image: '🎁',
    sellingPoints: ['视频会员', '音乐会员', '购物折扣']
  }],
  // 转化目标（按业务单元分类）
...
function App() {
    ...
    reader.onload = ev => {
      ...
      img.onload = () => {
        const w = img.naturalWidth,
          h = img.naturalHeight;
        if (Math.abs(w - h) > 2) {
          notify('宽高比需为 1:1', 'error');
          return;
        }
        if (Math.abs(w - 108) > 2 || Math.abs(h - 108) > 2) {
          notify('尺寸需为 108×108', 'error');
          return;
        }
  ...
  const overallProgress = (() => {
    const checks = [taskName !== '', businessUnit !== '', selectedAccountIds.length > 0, productAllocMode === 'shared' ? specificProduct !== '' : selectedAccountIds.length > 0 && selectedAccountIds.every(id => !!perAccountProduct[id]), conversionGoal !== '', targetingSource === 'package' ? selectedTargetingPackages.length > 0 : geoSelectedProvinces.length > 0 || geoMode === 'unlimited', bidStrategy === 'max_convert' ? true : bidAmount !== '', selectedMaterials.length > 0, selectedCopies.length > 0, unitName !== ''];
  ...
  const productInfoTargets = (() => {
    ...
    return selectedAccountIds.filter(accountId => !!perAccountProduct[accountId]).map(accountId => ({
      key: accountId,
      productId: perAccountProduct[accountId],
      accountId
    }));
  })();
  ...
  useEffect(() => {
    ensureAccountGroups(selectedAccountIds);
  }, [selectedAccountIds]);
      ...
      if (saved) {
          ...
          if (data.productAllocMode === 'per_account' && data.perAccountProduct) {
            ...
            Object.keys(data.perAccountProduct).forEach(accountId => {
              ...
              if (pid && !migratedMap[accountId] && data.productInfoMap[pid]) {
                migratedMap[accountId] = {
                  ...data.productInfoMap[pid],
                  images: [...(data.productInfoMap[pid].images || [])],
                  sellingPoints: [...(data.productInfoMap[pid].sellingPoints || [])]
                };
              }
        ...
        if (!data.productInfoMap && (data.extraProductImages || data.extraProductNames || data.extraSellingPoints || data.extraSellingPointsInput !== undefined)) {
          ...
          if (firstPid) {
            setProductInfoMap({
              [firstPid]: {
                extraName: data.extraProductNames && data.extraProductNames[0] || '',
                images: data.extraProductImages || [],
                sellingPoints: data.extraSellingPoints || []
              }
            });
            if (data.extraSellingPointsInput !== undefined) setSellingPointsInputs({
              [firstPid]: data.extraSellingPointsInput
            });
          }
        }
  ...
  }, [selectedAccountIds, unitName, selectedMaterials, selectedCopies, selectedTargetingPackages]);
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "w-28 text-left text-sm font-medium text-gray-700 flex-shrink-0"
  }, "选择主体 ", /*#__PURE__*/React.createElement("span", {
    className: "text-red-500"
    ...
    value: "lead"
  }, "销售线索"))), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center gap-3 mb-5"
  }, /*#__PURE__*/React.createElement("label", {
    className: "w-28 text-left text-sm font-medium text-gray-700 flex-shrink-0"
  }, "营销场景"), /*#__PURE__*/React.createElement("div", {
    className: "flex gap-3"
  }, /*#__PURE__*/React.createElement("span", {
    ...
    className: "fas fa-check-circle text-blue-500 mr-2 text-xs"
  }), /*#__PURE__*/React.createElement("span", null, "短视频+图文")))), /*#__PURE__*/React.createElement("div", {
    className: "mb-5"
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex items-center gap-3"
  }, /*#__PURE__*/React.createElement("label", {
    ...
    className: "flex items-center cursor-pointer px-4 py-2 border rounded-lg text-sm",
    style: {
      borderColor: productAllocMode === 'shared' ? '#1890ff' : '#e5e7eb',
      background: productAllocMode === 'shared' ? '#eff6ff' : '#fff'
    }
  }, /*#__PURE__*/React.createElement("input", {
    type: "radio",
    name: "productAllocMode",
    checked: productAllocMode === 'shared',
    onChange: () => setProductAllocMode('shared'),
    className: "w-4 h-4 mr-2 text-blue-600"
  }), /*#__PURE__*/React.createElement("span", null, "全账户共用")), /*#__PURE__*/React.createElement("label", {
    className: "flex items-center cursor-pointer px-4 py-2 border rounded-lg text-sm",
    style: {
      borderColor: productAllocMode === 'per_account' ? '#1890ff' : '#e5e7eb',
      background: productAllocMode === 'per_account' ? '#eff6ff' : '#fff'
    }
  }, /*#__PURE__*/React.createElement("input", {
    type: "radio",
    name: "productAllocMode",
    checked: productAllocMode === 'per_account',
    ...
    className: "w-48 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
  }, MOCK.productLibrary.map(sp => /*#__PURE__*/React.createElement("option", {
    key: sp.id,
    value: sp.id
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "text-xs font-semibold text-gray-900 truncate mb-1.5",
      title: acc ? acc.name : accountId
    }, acc ? acc.name : accountId), /*#__PURE__*/React.createElement("select", {
      value: pid,
      onChange: e => setPerAccountProduct(prev => ({
        ...prev,
      ...
      value: ""
    }, "请选择商品"), MOCK.productLibrary.map(sp => /*#__PURE__*/React.createElement("option", {
      key: sp.id,
      value: sp.id
    }, sp.name))));
  }))), /*#__PURE__*/React.createElement("div", {
  ...
  }, "排期与预算出价"), /*#__PURE__*/React.createElement("div", {
    className: "mb-5"
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex items-center gap-6 flex-wrap"
  }, /*#__PURE__*/React.createElement("span", {
    className: "text-sm font-medium text-gray-700"
  }, "投放时间"), /*#__PURE__*/React.createElement("label", {
    className: "flex items-center cursor-pointer"
  }, /*#__PURE__*/React.createElement("input", {
    type: "radio",
    name: "date_type",
    ...
    className: "text-sm font-medium text-gray-700"
  }, "投放时段"), /*#__PURE__*/React.createElement("label", {
    className: "flex items-center cursor-pointer"
  }, /*#__PURE__*/React.createElement("input", {
    type: "radio",
    name: "time_mode",
    ...
    className: "text-sm"
  }, "指定多时段"))), 投放时段模式 === 'time_range' && /*#__PURE__*/React.createElement("div", {
    className: "flex gap-4 items-center p-4 bg-gray-50 rounded-lg border border-gray-200 max-w-xl mt-3"
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex-1"
  }, /*#__PURE__*/React.createElement("label", {
    ...
    className: "far fa-clock mr-1"
  }), "配置广告素材、广告文案、产品与创意组件")), /*#__PURE__*/React.createElement("div", {
    className: "p-6 space-y-6"
  }, /*#__PURE__*/React.createElement("div", {
    ...
    // 每产品一套配置面板（名称/主图/卖点），单产品与多产品 Tabs 共用同一套面板
    const _renderPanel = (pid, defaultName, defaultImage, imgs, points, allImages) => {
      const _pi = productInfoMap[pid] || {};
      const extraName = _pi.extraName || '';
      return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
      ...
      }, extraName.length, "/20"))), /*#__PURE__*/React.createElement("div", {
        className: "mb-5"
      }, /*#__PURE__*/React.createElement("div", {
        className: "flex items-center gap-1.5 mb-2"
      }, /*#__PURE__*/React.createElement("span", {
        className: "text-sm font-medium text-gray-700"
      }, "产品主图"), /*#__PURE__*/React.createElement("span", {
        className: "text-red-500"
      }, "*")), /*#__PURE__*/React.createElement("div", {
        className: "text-xs text-gray-500 mb-2"
      }, "已添加：", /*#__PURE__*/React.createElement("span", {
        className: "text-gray-700"
      }, allImages.length), "/11"), /*#__PURE__*/React.createElement("div", {
        className: "flex flex-wrap gap-2 mb-2"
      }, allImages.map((em, i) => /*#__PURE__*/React.createElement("div", {
        key: i,
        ...
        type: "button",
        onClick: () => setProductInfoMap(prev => {
          const cur = prev[pid] || {};
          return {
            ...prev,
            [pid]: {
              ...cur,
              images: (cur.images || []).filter((_, idx) => idx !== i - 1)
            }
          };
        }),
        className: "absolute -top-1 -right-1 w-4 h-4 bg-gray-700 text-white rounded-full text-[10px] flex items-center justify-center hover:bg-red-500"
      }, /*#__PURE__*/React.createElement("i", {
        className: "fas fa-times"
      })))), allImages.length < 11 && /*#__PURE__*/React.createElement("button", {
        type: "button",
        ...
        className: "hidden",
        onChange: handleImageFileChange(pid)
      }), /*#__PURE__*/React.createElement("div", {
        className: "flex items-start gap-1.5 text-xs text-gray-400 mt-1"
      }, /*#__PURE__*/React.createElement("i", {
        className: "fas fa-image mt-0.5"
      }), /*#__PURE__*/React.createElement("span", null, "支持格式：jpg / png / jpeg；宽高比 1:1；大小 ≤ 1M；尺寸 108×108 ≤ 尺寸 ≤ 108×108")), imgs.length === 0 && /*#__PURE__*/React.createElement("div", {
        className: "flex items-start gap-1.5 text-xs text-orange-500 mt-1.5"
      }, /*#__PURE__*/React.createElement("i", {
        className: "fas fa-exclamation-circle mt-0.5"
      }), /*#__PURE__*/React.createElement("span", null, "产品主图必须至少手动上传 1 张"))), /*#__PURE__*/React.createElement("div", {
        className: "mb-3"
      }, /*#__PURE__*/React.createElement("div", {
        className: "flex items-center gap-1.5 mb-2"
      }, /*#__PURE__*/React.createElement("span", {
        className: "text-sm font-medium text-gray-700"
      }, "产品卖点"), /*#__PURE__*/React.createElement("span", {
        className: "text-red-500"
      }, "*"), /*#__PURE__*/React.createElement("i", {
        className: "far fa-question-circle text-gray-400 text-xs",
        title: "每条 6-9 个字，空格分隔多个，回车提交"
      })), /*#__PURE__*/React.createElement("textarea", {
        value: sellingPointsInputs[pid] || '',
        onChange: e => setSellingPointsInputs(prev => ({
          ...prev,
          [pid]: e.target.value
        })),
        onKeyDown: e => {
          if (e.key === 'Enter') {
            e.preventDefault();
            const tokens = (sellingPointsInputs[pid] || '').split(/\s+/).map(s => s.trim()).filter(Boolean);
            if (tokens.length === 0) return;
            if (points.length + tokens.length > 10) {
              notify('产品卖点最多 10 条', 'error');
              return;
            }
            const invalid = tokens.find(t => t.length < 6 || t.length > 9);
            if (invalid) {
              notify(`「${invalid}」长度需 6-9 字`, 'error');
              return;
            }
            const dup = tokens.find(t => points.includes(t));
            if (dup) {
              notify(`「${dup}」已存在`, 'error');
              return;
            }
            setProductInfoMap(prev => {
              const cur = prev[pid] || {};
              return {
                ...
                [pid]: {
                  ...cur,
                  sellingPoints: [...(cur.sellingPoints || []), ...tokens]
                }
              };
            });
            setSellingPointsInputs(prev => ({
              ...prev,
              [pid]: ''
            }));
          }
      ...
      }, points.length, "/10"), points.length > 0 && /*#__PURE__*/React.createElement("div", {
        className: "flex flex-wrap gap-1.5 mt-2"
      }, points.map((sp, i) => /*#__PURE__*/React.createElement("span", {
        key: i,
        className: "inline-flex items-center gap-1 text-xs bg-blue-50 text-blue-700 border border-blue-200 rounded px-2 py-0.5"
      }, sp, /*#__PURE__*/React.createElement("button", {
        type: "button",
        onClick: () => setProductInfoMap(prev => {
          const cur = prev[pid] || {};
          return {
    ...
    if (_targets.length <= 1) {
      ...
      }, "产品信息"), _renderPanel(_target.key, _defaultName, _defaultImage, _imgs, _points, _allImages));
    }
    ...
    return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("h4", {
      ...
      return /*#__PURE__*/React.createElement("button", {
        ...
        style: {
          ...
          alignItems: 'center',
          gap: 6,
          borderRadius: _isActive ? '8px 8px 0 0' : '8px',
          border: '1px solid ' + (_isActive ? '#1890ff' : '#e5e7eb'),
      ...
      }, target.accountId, /*#__PURE__*/React.createElement("span", {
        style: {
          width: 8,
          height: 8,
          borderRadius: '50%',
          background: _done ? '#22c55e' : _started ? '#ef4444' : '#d1d5db',
          flexShrink: 0
        }
      }));
    ...
    className: "far fa-question-circle text-gray-400 text-xs",
    title: "最多10个行动号召，每个2-4个字，可空格分隔，回车(Enter)提交"
  })), /*#__PURE__*/React.createElement("textarea", {
    value: ctaInput,
    ...
    onKeyDown: e => {
      if (e.key === 'Enter') {
        e.preventDefault();
        const tokens = ctaInput.split(/\s+/).map(s => s.trim()).filter(Boolean);
        if (tokens.length === 0) return;
        if (ctaList.length + tokens.length > 10) {
          notify('行动号召最多 10 条', 'error');
          return;
        }
        const invalid = tokens.find(t => t.length < 2 || t.length > 4);
        if (invalid) {
          notify(`「${invalid}」长度需 2-4 个字`, 'error');
          return;
        }
        const dup = tokens.find(t => ctaList.includes(t));
        if (dup) {
          notify(`「${dup}」已存在`, 'error');
          return;
        }
        setCtaList([...ctaList, ...tokens]);
        setCtaInput('');
      }
    },
    placeholder: "最多10个行动号召，每个2-4个字，可空格分隔，回车(Enter)提交",
    rows: 3,
    className: "w-full max-w-md px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 resize-y"
  }), /*#__PURE__*/React.createElement("div", {
    className: "text-xs text-gray-400 mt-1"
    ...
    key: i,
    className: "inline-flex items-center gap-1 text-xs bg-gray-100 text-gray-800 border border-gray-200 rounded px-2 py-0.5"
    ...
    return /*#__PURE__*/React.createElement("label", {
      ...
      onChange: () => {
        if (checked) {
          ...
          setSelectedCreativeComponents([...selectedCreativeComponents, {
            id: cc.id,
            name: cc.name
          }]);
        }
    ...
    return /*#__PURE__*/React.createElement("tr", {
      ...
      className: "px-3 py-2 align-top text-gray-700"
    }, r.creaSucc, "成 / ", r.creaFail, "败"), /*#__PURE__*/React.createElement("td", {
      className: "px-3 py-2 align-top"
    }, reasons.length ? reasons.map((rr, ri) => /*#__PURE__*/React.createElement("span", {
      key: ri,
      className: "inline-block px-2 py-1 mb-1 mr-1 rounded bg-red-50 text-red-500 text-xs"
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, "广告文案"))), /*#__PURE__*/React.createElement("div", {
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        className: "text-xs text-gray-400"
      }, "子账户 ID"))), /*#__PURE__*/React.createElement("div", {
      ...
      const configItems = (() => {
        ...
        items.push({
          label: '选择账户',
          value: selectedAccountIds.length > 0 ? selectedAccountIds.length + ' 个' : '未选择',
          required: true,
          ok: selectedAccountIds.length > 0
        });
        items.push({
          label: '业务单元',
          value: bu ? bu.name : '未设置',
          required: false,
          ok: !!bu
        });
        items.push({
          label: '营销目的',
          value: mo ? mo.name : '未设置',
          required: false,
          ok: !!mo
        });
        items.push({
          label: '产品',
          value: prod ? prod.name : '未设置',
          required: true,
          ok: !!prod
        });
        items.push({
          label: '转化目标',
          value: conv ? conv.name : '未设置',
          required: true,
          ok: !!conv
        });
        items.push({
        ...
        items.push({
          label: '日预算',
          value: dailyBudget !== '' ? '¥' + dailyBudget : '未设置',
          required: false,
          ok: dailyBudget !== ''
        });
        items.push({
        ...
        items.push({
          label: '营销产品',
          value: (productAllocMode === 'shared' ? (MOCK.productLibrary.find(p => p.id === specificProduct) || {}).name : '分账户定制') || '未设置',
          required: false,
          ok: !!(productAllocMode === 'shared' ? specificProduct : Object.keys(perAccountProduct).length > 0)
        });
        items.push({
          label: '行动号召',
          value: ctaList.length > 0 ? ctaList.length + ' 条' : '未设置',
          required: true,
          ok: ctaList.length > 0
        });
        items.push({
          label: '来源',
          value: sourceText || '未设置',
          required: false,
          ok: !!sourceText
        });
        items.push({
        ...
        items.push({
          label: '运行模式',
          value: runMode === 'scheduled' ? '定时运行' : '立即运行',
          required: false,
          ok: true
        });
        if (runMode === 'scheduled') items.push({
          label: '定时时间',
          value: scheduledDate && scheduledTime ? scheduledDate + ' ' + scheduledTime : '未设置',
          required: true,
          ok: !!(scheduledDate && scheduledTime)
        });
        items.push({
          label: '投放日期',
          value: 投放日期类型 === 'long_term' ? '从今天起长期投放' : '设置开始和结束日期',
          required: false,
          ok: true
        });
        items.push({
        ...
        items.push({
          label: '推官链接',
          value: getDefaultLandingPage(selectedAccountIds[0] || '') ? '已设置' : '未设置',
          required: false,
          ok: true
        });
        return items;
      ...
      return /*#__PURE__*/React.createElement("div", {
      ...
      }), "关键配置摘要", missCount > 0 ? /*#__PURE__*/React.createElement("span", {
        className: "ml-2 text-xs text-red-500 font-normal"
      }, "（", missCount, " 项必填未配置）") : /*#__PURE__*/React.createElement("span", {
        className: "ml-2 text-xs text-green-600 font-normal"
      }, "（必填项均已配置）")), /*#__PURE__*/React.createElement("div", {
        ...
        }, it.label, "："), miss ? /*#__PURE__*/React.createElement("span", {
          className: "font-medium text-red-500"
        }, it.value, /*#__PURE__*/React.createElement("span", {
          className: "ml-1 text-xs border border-red-300 rounded px-1"
        }, "未配置")) : /*#__PURE__*/React.createElement("span", {
