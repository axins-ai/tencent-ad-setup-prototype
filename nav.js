...
function TaskListPage(props) {
  ...
  function handleDeleteTask(id) {
    ...
    setTasks(newTasks);
    try {
      localStorage.setItem('ad_tasks', JSON.stringify(newTasks));
    } catch (e) {}
  }
  ...
  }, '批创工具'), React.createElement('p', {
    className: 'text-sm text-gray-500 mt-1'
  }, '管理批量创意搭建任务')), React.createElement('button', {
    onClick: () => setCurrentView('channel-select'),
    className: 'btn-primary text-base px-6 py-3'
  }, React.createElement('i', {
    className: 'fas fa-plus mr-2'
  }), '新建任务')), tasks.length === 0 && React.createElement('div', {
    className: 'text-center py-20 text-gray-400'
  }, React.createElement('div', {
    className: 'text-6xl mb-4'
  }, '📋'), React.createElement('p', {
    className: 'text-lg'
  }, '暂无任务'), React.createElement('p', {
    className: 'text-sm mt-2'
  }, '点击「新建任务」开始创建批量创意搭建')), tasks.length > 0 && React.createElement('div', {
  ...
  }, '渠道：' + (task.channelName || task.channel)), React.createElement('p', {
    className: 'text-xs text-gray-400 mt-1'
  }, '创建于 ' + new Date(task.createdAt).toLocaleDateString())), React.createElement('button', {
...
function ChannelSelectionPage(props) {
  ...
  return React.createElement('div', {
    ...
    className: 'text-sm'
  }, '返回任务列表')), React.createElement('div', {
    className: 'mb-8'
  }, React.createElement('h2', {
    className: 'text-2xl font-bold text-gray-900 mb-2'
  }, '选择投放渠道'), React.createElement('p', {
    className: 'text-sm text-gray-500'
  }, '选择要投放的渠道，进入对应的广告搭建表单')), React.createElement('div', {
    className: 'space-y-4'
  }, channels.map(ch => React.createElement('button', {
    key: ch.id,
    ...
    className: 'text-3xl'
  }, ch.icon), React.createElement('div', {
    className: 'flex-1'
  }, React.createElement('h3', {
    className: 'font-bold text-gray-900 text-lg'
  }, ch.name), React.createElement('p', {
    className: 'text-sm text-gray-500 mt-1'
  }, ch.desc)), React.createElement('i', {
