    ...
    reader.onload = (ev) => {
      ...
      img.onload = () => {
        ...
        setProductInfoMap(prev => {
          ...
          if (imgs.length >= 10) { notify('产品主图最多 11 张', 'error'); return prev; }
          return { ...prev, [pid]: { ...cur, images: [...imgs, dataUrl] } };
        });
        e.target.value = '';
      ...
      if (saved) {
        ...
        if (!data.productInfoMap && (data.extraProductImages || data.extraProductNames || data.extraSellingPoints || data.extraSellingPointsInput !== undefined)) {
          ...
          if (firstPid) {
            ...
            if (data.extraSellingPointsInput !== undefined) setSellingPointsInputs({ [firstPid]: data.extraSellingPointsInput });
          }
        }
  ...
  const doSaveForm = () => {
    ...
    try {
      const data = {
        ...
        productInfoMap, sellingPointsInputs,
  ...
  const handleRun = () => {
    ...
    try {
      if (window.parent && window.parent !== window) {
        window.parent.postMessage({ type: 'TASK_FORM_SAVE' }, '*');
      }
    } catch (e) {}
                  ...
                  <div className="absolute z-50 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg" style={{ width: '560px' }}>
                    ...
                    <div className="max-h-56 overflow-y-auto">
                      {(MOCK.conversionsByBusinessUnit[businessUnit] || []).length === 0 ? (
                        ...
                        (MOCK.conversionsByBusinessUnit[businessUnit] || []).map(conv => {
        ...
        <div id="section-creative" className="">
          ...
          <div className="p-6 space-y-6">
                  ...
                  <div>
                    ...
                    <div className="mb-5">
                      ...
                      <div className="relative mb-2 max-w-md">
                        <input type="text" value={defaultName} readOnly disabled className="w-full px-3 py-2 bg-gray-100 border border-gray-200 rounded-lg text-sm text-gray-600 cursor-not-allowed" />
                      </div>
                      {/* 第二个：可额外添加（选填，表单校验逻辑控制，最多 1 条） */}
                      <div className="relative max-w-md">
                        <input type="text" maxLength={20} value={extraName} onChange={e => {
                          const v = e.target.value;
                          setProductInfoMap(prev => ({ ...prev, [pid]: { ...(prev[pid] || {}), extraName: v } }));
                        ...
                        <span className="absolute right-3 text-xs text-gray-400 whitespace-nowrap" style={{ top: '50%', transform: 'translateY(-50%)' }}>{extraName.length}/20</span>
                      </div>
                    </div>
                    ...
                    <div className="mb-5">
                      ...
                      <div className="text-xs text-gray-500 mb-2">已添加：<span className="text-gray-700">{allImages.length}</span>/11</div>
                      <div className="flex flex-wrap gap-2 mb-2">
                        {allImages.map((em, i) => (
                          <div key={i} className="relative w-16 h-16 border border-gray-200 rounded-lg bg-white flex items-center justify-center text-2xl shadow-sm overflow-hidden">
                            ...
                            {i > 0 && <button type="button" onClick={() => setProductInfoMap(prev => {
                              const cur = prev[pid] || {};
                              return { ...prev, [pid]: { ...cur, images: (cur.images || []).filter((_, idx) => idx !== i - 1) } };
                            })} className="absolute -top-1 -right-1 w-4 h-4 bg-gray-700 text-white rounded-full text-[10px] flex items-center justify-center hover:bg-red-500"><i className="fas fa-times"></i></button>}
                          </div>
                        ))}
                        {allImages.length < 11 && (
                      ...
                      <input ref={imageUploadRef} type="file" accept="image/jpeg,image/png,.jpg,.jpeg,.png" className="hidden" onChange={handleImageFileChange(pid)} />
                    ...
                    <div className="mb-3">
                      ...
                      <textarea
                        ...
                        onChange={e => setSellingPointsInputs(prev => ({ ...prev, [pid]: e.target.value }))}
                        onKeyDown={e => {
                          if (e.key === 'Enter') {
                            e.preventDefault();
                            const tokens = (sellingPointsInputs[pid] || '').split(/\s+/).map(s => s.trim()).filter(Boolean);
                            if (tokens.length === 0) return;
                            if (points.length + tokens.length > 10) { notify('产品卖点最多 10 条', 'error'); return; }
                            ...
                            setProductInfoMap(prev => {
                              const cur = prev[pid] || {};
                              return { ...prev, [pid]: { ...cur, sellingPoints: [...(cur.sellingPoints || []), ...tokens] } };
                            });
                            setSellingPointsInputs(prev => ({ ...prev, [pid]: '' }));
                          }
                        }}
                      ...
                      />
                      <div className="text-xs text-gray-400 mt-1">{points.length}/10</div>
                      {points.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mt-2">
                          {points.map((sp, i) => (
                            <span key={i} className="inline-flex items-center gap-1 text-xs bg-blue-50 text-blue-700 border border-blue-200 rounded px-2 py-0.5">
                              {sp}
                              <button type="button" onClick={() => setProductInfoMap(prev => {
                                const cur = prev[pid] || {};
                                return { ...prev, [pid]: { ...cur, sellingPoints: (cur.sellingPoints || []).filter((_, idx) => idx !== i) } };
                              })} className="text-blue-400 hover:text-red-500"><i className="fas fa-times"></i></button>
                            </span>
                          ))}
                        </div>
                      )}
            ...
            <div>
              ...
              {/* 行动号召（样式与产品卖点一致：textarea + 回车提交 + 空格分隔） */}
              <div className="mb-5">
                <div className="flex items-center gap-1.5 mb-2">
                ...
                <textarea
                  ...
                  onKeyDown={e => {
                    if (e.key === 'Enter') {
                      ...
                      if (tokens.length === 0) return;
                      if (ctaList.length + tokens.length > 10) { notify('行动号召最多 10 条', 'error'); return; }
                      const invalid = tokens.find(t => t.length < 2 || t.length > 4);
                      if (invalid) { notify(`「${invalid}」长度需 2-4 个字`, 'error'); return; }
                  ...
                  className="w-full max-w-md px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 resize-y"
                />
                <div className="text-xs text-gray-400 mt-1">{ctaList.length}/10</div>
                {ctaList.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {ctaList.map((c, i) => (
                      <span key={i} className="inline-flex items-center gap-1 text-xs bg-gray-100 text-gray-800 border border-gray-200 rounded px-2 py-0.5">
                        {c}
                        <button type="button" onClick={() => setCtaList(ctaList.filter((_, idx) => idx !== i))} className="text-gray-400 hover:text-red-500"><i className="fas fa-times"></i></button>
                      </span>
                    ))}
                  </div>
                )}
