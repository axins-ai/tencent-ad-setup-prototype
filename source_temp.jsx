    ...
    <div className="border border-gray-300 rounded-lg overflow-hidden">
      <table className="w-full border-collapse" style={{fontSize:'13px'}}>
        <thead>
          <tr className="bg-gray-50">
            <th className="border-r border-b border-gray-200 px-3 py-2 text-center font-medium text-gray-600 w-20">星期\时间</th>
            <th colSpan="12" className="text-center font-medium text-gray-600 px-2 py-2">00:00 - 12:00</th>
            <th colSpan="12" className="text-center font-medium text-gray-600 px-2 py-2 border-l border-gray-200">12:00 - 24:00</th>
          </tr>
          <tr className="bg-gray-50">
            <th className="border-r border-b border-gray-200"></th>
            {HOURS.map(h => (
              <th key={h} className={`px-1 py-1 text-center text-xs text-gray-500 font-normal ${h === 12 ? 'border-l border-gray-200' : ''}`}>
                {h}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {DAYS.map((day, di) => (
            <tr key={di}>
              <td className="border-r border-b border-gray-200 px-3 py-1 text-center font-medium text-gray-700 bg-gray-50 whitespace-nowrap">
                {day}
              </td>
              {HOURS.map(hi => {
                const key = `${di}-${hi}`;
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-gray-50 border-t border-gray-200">
        <div className="flex items-center gap-4 text-xs text-gray-500">
          <span className="flex items-center gap-1"><span className="inline-block w-3.5 h-3.5 rounded-sm bg-blue-500"></span>已选</span>
          <span className="flex items-center gap-1"><span className="inline-block w-3.5 h-3.5 rounded-sm bg-gray-100 border border-gray-300"></span>未选</span>
          <span>可拖动鼠标选择时间段</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-xs text-gray-600">已选 {selectedCount} 个时间段</span>
          <button onClick={clearAll} className="text-xs text-blue-600 hover:text-blue-800 cursor-pointer">清空</button>
        </div>
      </div>
    </div>
  );
...
function App() {
  ...
  const [selectedChannel, setSelectedChannel] = useState('gdt');
  const [tasks, setTasks] = useState(function() {
    try { return JSON.parse(localStorage.getItem('ad_tasks') || '[]'); } catch(e) { return []; }
  });
                    ...
                    <div className="flex items-center gap-1 mb-3">
                      ...
                      <div className="flex flex-wrap items-center gap-x-5 gap-y-1 ml-2">
                        ...
                        <label className="flex items-center cursor-pointer ml-1">
                          <input
                            type="checkbox"
                            checked={ageSelections.includes('custom')}
                            onChange={e => {
                              let next = ageSelections.filter(k => k !== 'unlimited');
                              if (e.target.checked && !next.includes('custom')) next.push('custom');
                              else next = next.filter(k => k !== 'custom');
                              setAgeSelections(next.length > 0 ? next : ['unlimited']);
                            }}
                            disabled={ageSelections.includes('unlimited')}
                            className="mr-1.5"
                          />
                          <span className={`text-sm ${ageSelections.includes('custom') ? 'text-gray-900 font-medium' : ageSelections.includes('unlimited') ? 'text-gray-300' : 'text-gray-700'}`}>自定义</span>
                          <i className="fas fa-info-circle text-gray-300 ml-1 text-xs cursor-help" title="自定义年龄段"></i>
                        </label>
                      </div>
                    </div>
                    {ageSelections.includes('custom') && (
                      <div className="ml-[52px] flex items-center gap-3 animate-fadeIn">
                        <input type="number" value={customAgeMin} onChange={e => setCustomAgeMin(e.target.value)} placeholder="最小年龄" className="w-24 px-3 py-1.5 border border-gray-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500" />
                        <span className="text-gray-400">-</span>
                        <input type="number" value={customAgeMax} onChange={e => setCustomAgeMax(e.target.value)} placeholder="最大年龄" className="w-24 px-3 py-1.5 border border-gray-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500" />
