...
function App() {
  ...
  // ===== 定向包管理（弹窗） =====
  const [showTargetingPkgModal, setShowTargetingPkgModal] = useState(false);
  const [targetingPkgModalView, setTargetingPkgModalView] = useState('list'); // 'list' | 'edit'
  const [editingPkgId, setEditingPkgId] = useState(null); // null = 新增，string = 编辑
  // 用户自建定向包（存 localStorage）
  const [userTargetingPackages, setUserTargetingPackages] = useState([]);
  // 弹窗内表单临时状态
  const [pkgFormName, setPkgFormName] = useState('');
  const [pkgFormGeoMode, setPkgFormGeoMode] = useState('unlimited');
  const [pkgFormGeoProvinces, setPkgFormGeoProvinces] = useState([]);
  const [pkgFormAge, setPkgFormAge] = useState('unlimited');
  const [pkgFormGender, setPkgFormGender] = useState('unlimited');
  const [pkgFormAudienceMode, setPkgFormAudienceMode] = useState('unlimited');
  const [pkgFormSelectedAudiences, setPkgFormSelectedAudiences] = useState([]);

  // 初始化：从 localStorage 读取用户自建定向包
  useEffect(() => {
    try {
      const saved = localStorage.getItem('ad_user_targeting_packages');
      if (saved) setUserTargetingPackages(JSON.parse(saved));
    } catch (e) {}
  }, []);
  const saveUserPackages = pkgs => {
    setUserTargetingPackages(pkgs);
    try {
      localStorage.setItem('ad_user_targeting_packages', JSON.stringify(pkgs));
    } catch (e) {}
  };

  // 所有可用定向包（MOCK + 用户自建）
  const getAllPackages = () => [...MOCK.targetingPackages, ...userTargetingPackages];

  // 打开弹窗
  const openTargetingModal = () => {
    setShowTargetingPkgModal(true);
    setTargetingPkgModalView('list');
  ...
  const startNewPkg = () => {
    setPkgFormName('');
    setPkgFormGeoMode('unlimited');
    setPkgFormGeoProvinces([]);
    setPkgFormAge('unlimited');
    setPkgFormGender('unlimited');
    setPkgFormAudienceMode('unlimited');
    setPkgFormSelectedAudiences([]);
    setEditingPkgId(null);
    setTargetingPkgModalView('edit');
  };

  // 编辑定向包：加载数据到表单
  const startEditPkg = pkgId => {
    const all = getAllPackages();
    const pkg = all.find(p => p.id === pkgId);
    if (!pkg) return;
    setPkgFormName(pkg.name || '');
    setPkgFormGeoMode(pkg.geoMode || 'unlimited');
    setPkgFormGeoProvinces(pkg.geoProvinces || []);
    setPkgFormAge(pkg.ageKey || 'unlimited');
    setPkgFormGender(pkg.genderKey || 'unlimited');
    setPkgFormAudienceMode(pkg.audienceMode || 'unlimited');
    setPkgFormSelectedAudiences(pkg.audienceIds || []);
  ...
  const savePkg = () => {
    if (!pkgFormName.trim()) {
      notify('请输入定向包名称', 'warning');
      return;
    }
    const mockIds = MOCK.targetingPackages.map(p => p.id);
    if (editingPkgId) {
      const updated = userTargetingPackages.map(p => p.id === edtingPkgId ? {
        ...p,
        name: pkgFormName,
        geoMode: pkgFormGeoMode,
        geoProvinces: pkgFormGeoProvinces,
        ageKey: pkgFormAge,
        genderKey: pkgFormGender,
        audienceMode: pkgFormAudienceMode,
        audienceIds: pkgFormSelectedAudiences
      } : p);
      saveUserPackages(updated);
    } else {
      const newPkg = {
        id: 'user_tp_' + Date.now(),
        name: pkgFormName,
        geoMode: pkgFormGeoMode,
        geoProvinces: pkgFormGeoProvinces,
        ageKey: pkgFormAge,
      ...
      saveUserPackages([...userTargetingPackages, newPkg]);
    }
    setTargetingPkgModalView('list');
    notify(editingPkgId ? '定向包已更新' : '定向包已创建', 'success');
  };

  // 删除定向包
  const deletePkg = pkgId => {
    if (!confirm('确定删除该定向包？')) return;
    const mockIds = MOCK.targetingPackages.map(p => p.id);
    if (mockIds.includes(pkgId)) {
      notify('系统内置定向包不能删除', 'warning');
      return;
    }
    const newUser = userTargetingPackages.filter(p => p.id !== pkgId);
    saveUserPackages(newUser);
    setSelectedTargetingPackages(selectedTargetingPackages.filter(id => id !== pkgId));
    notify('定向包已删除', 'success');
  };
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    const tp = getAllPackages().find(t => t.id === tpId);
    ...
    className: "fas fa-th-list mr-1"
  }), " 选择定向包"), selectedTargetingPackages.length === 0 && /*#__PURE__*/React.createElement("p", {
    className: "text-xs text-orange-500 mt-1"
  }, /*#__PURE__*/React.createElement("i", {
    className: "fas fa-exclamation-circle mr-1"
  }), "请至少选择一个定向包")), /*#__PURE__*/React.createElement("div", {
    className: "border-t pt-4"
  }, /*#__PURE__*/React.createElement("h3", {
    className: "text-md font-semibold text-gray-900 mb-3"
